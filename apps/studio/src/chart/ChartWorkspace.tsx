...
export function ChartWorkspace({ project, onProject, onNewProject, onProjectSettings, autosaveState, isDarkMode, compile, outputStatus, outputError, authoredVerdict, focusedRowId, onFocusedRowId }: {
  ...
  return <main className="chart-workspace">
    <div className="chart-head"><div><span className="eyebrow">Colorwork project</span><h1>{project.title}</h1></div><div className="chart-facts"><span>{chart.width} x {chart.height}</span><span>{chart.palette.length} colors</span><span>Row 1 bottom</span><span className={`autosave-state ${autosaveState}`} title={autosaveState === 'failed' ? 'Local recovery failed' : autosaveState === 'saving' ? 'Saving locally' : 'Saved locally'}>{autosaveState === 'failed' ? <CloudOff size={13} /> : autosaveState === 'saving' ? <LoaderCircle className="spin" size={13} /> : <Cloud size={13} />}{autosaveState === 'failed' ? 'Save failed' : autosaveState === 'saving' ? 'Saving' : 'Saved'}</span><button className="chart-file-button" type="button" onClick={onNewProject} title="New project" aria-label="New project"><Plus size={15} /></button><button className="chart-file-button" type="button" onClick={onProjectSettings} title="Project settings" aria-label="Project settings"><Settings2 size={15} /></button><button className="chart-file-button" type="button" onClick={() => projectInput.current?.click()} title="Open project" aria-label="Open project"><Upload size={15} /></button><button className="chart-file-button" type="button" onClick={saveProject} title="Save project" aria-label="Save project"><Download size={15} /></button></div></div>
