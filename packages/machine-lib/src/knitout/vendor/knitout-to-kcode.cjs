...
//PATCH (knitlab2): module-level presser state — set by x-presser-* ops
//inside knitoutToPasses, snapshotted onto each Pass at construction so
//passesToKCode() can read it for auto-carriage-move emission.
let _presserSpeed = null;
let _presserRoller = null;
...
function Pass(info) {
	...
	//PATCH (knitlab2): snapshot presser state — used by carriageMove()
	//to control the speed/roller of auto-inserted carriage moves.
	this.presserSpeed = _presserSpeed;
	this.presserRoller = _presserRoller;
...
function knitoutToPasses(knitout, knitoutFile) {
	...
	//PATCH (knitlab2): reset module-level presser state for this run.
	_presserSpeed = null;
	_presserRoller = null;
	...
	for ( ; lineIdx < lines.length; ++lineIdx) {
		...
		} else if (op === 'x-presser-speed') { //PATCH (knitlab2): one-shot presser speed for next auto-move
			if (args.length !== 1) throw `${knitoutFile}:${lineIdx+1} ERROR: x-presser-speed takes one argument.`;
			if (!/^[+-]?\d*\.?\d+$/.test(args[0])) throw `${knitoutFile}:${lineIdx+1} ERROR: x-presser-speed must be a number.`;
			_presserSpeed = parseFloat(args[0]);
		} else if (op === 'x-presser-roller') { //PATCH (knitlab2): one-shot presser roller for next auto-move
			if (args.length !== 1) throw `${knitoutFile}:${lineIdx+1} ERROR: x-presser-roller takes one argument.`;
			if (!/^[+-]?\d*\.?\d+$/.test(args[0])) throw `${knitoutFile}:${lineIdx+1} ERROR: x-presser-roller must be a number.`;
			_presserRoller = parseFloat(args[0]);
		} else if (op === 'x-park-carriage') { //PATCH (knitlab2): emit a synthetic carriage move at current presser values
			//Adds a "park" marker pass that the emitter renders as an
			//empty Kn-Kn pass with no carrier — same shape as the
			//auto-inserted move but explicitly requested.
			passes.push(new Pass({
				type:{kcode:'Kn-Kn'},
				racking:racking,
				speed:speed,
				roller:roller,
				direction:DIRECTION_RIGHT,  //will be flipped by emitter based on nextDirection
				carriers:[],
				slots:{},
				sizes:{},
			}));
			passes[passes.length-1].isPark = true;
...
function passesToKCode(headers, passes, kcFile) {
	...
	//PATCH (knitlab2): nextPass is the kpass that triggered this auto-move.
	//Its `presserSpeed` / `presserRoller` (snapshotted from x-presser-*
	//ops at parse time) control the speed/roller emitted on the move.
	//Fallback: use the next pass's own speed/roller (kpass.speed/roller),
	//then 300/0 as the historical default.
	function carriageMove(nextPass) {
		...
		//PATCH (knitlab2): resolve speed/roller. For an isPark pass
		//(explicit `x-park-carriage`), use the speed/roller captured on
		//the pass itself (set from the most recent x-speed-number /
		//x-roller-advance). For auto-inserted moves (no isPark), use
		//the next pass's presser settings, falling back to the legacy
		//300/0 default if no presser values are set.
		let mvSpeed = 300;
		let mvRoller = 0;
		if (nextPass && nextPass.isPark) {
			mvSpeed = nextPass.speed;
			mvRoller = nextPass.roller;
		} else {
			if (nextPass && nextPass.presserSpeed !== null && nextPass.presserSpeed !== undefined) {
				mvSpeed = nextPass.presserSpeed;
			}
			if (nextPass && nextPass.presserRoller !== null && nextPass.presserRoller !== undefined) {
				mvRoller = nextPass.presserRoller;
			}
		}
		let pass = {
			...
			roller:mvRoller,
			isAutoMove:true,
			//PATCH (knitlab2): App-style auto-moves keep the
			//"automatically inserted carriage move" comment; explicit
			//`x-park-carriage` moves (nextPass.isPark) omit it.
			comment: (nextPass && nextPass.isPark) ? undefined : "automatically inserted carriage move"
		};
		if (pass.comment === undefined) delete pass.comment;
	...
	passes.forEach(function(pass){
		//PATCH (knitlab2): x-park-carriage emits an explicit carriage move.
		if (pass.isPark) {
			carriageMove(pass);
			return;
		}
		...
		if (pass.type === TYPE_XFER_FOUR_PASS || pass.type === TYPE_XFER_TWO_PASS) {
			...
			function makeXferPass(direction, fromBed, toBed, checkNeedleFn, comment) {
				let xpass = {
					...
					//PATCH (knitlab2): propagate presser settings to any
					//auto-move that gets inserted before this transfer.
					presserSpeed: pass.presserSpeed,
					presserRoller: pass.presserRoller,
				};
			...
			let kpass = {
				...
				//roller:100,
				roller: pass.roller,
				//PATCH (knitlab2): propagate presser settings to the
				//upcoming auto-move (carriageMove(kpass)).
				presserSpeed: pass.presserSpeed,
				presserRoller: pass.presserRoller,
			};
	...
	out("HOME");
	out("RACK:0");
	//PATCH (knitlab2): use basename (last path segment) so the header
	//matches the Kniterate App's output (`// command.kc`).
	const kcBaseName = String(kcFile).replace(/\\/g, '/').split('/').pop();
	out(`// ${kcBaseName}`); //add name of .kc file as comment in header, since it needs to be changed to 'command.kc' for the machine to read it (so can remember original name)

	let lastRACK = 0.0;
	//PATCH (knitlab2): row-counter state machine that emits App-style
	//`// row: N` / `// rows: N-(N+1)` labels after the 3 `//` lines.
	//Algorithm (derived empirically from reference/swatch.kc):
	// - Regular knit passes advance counter by 1.
	// - `<<` double-bed (interlock-like) and transfer passes use the
	//   "rows: N-(N+1)" span label; counter still advances by 1.
	// - Transfer pair: 2nd transfer shares label with the 1st.
	// - Auto-moves advance the counter; the next regular pass usually
	//   *shares* the auto-move's label (no counter advance), EXCEPT in
	//   the pattern [auto, real, auto] where the middle real takes a
	//   fresh row (no share).
	// - Consecutive auto-moves share the same row.
	// - Once carrier-out Tu-Tu starts, counter freezes (bind-off).
	let rowCounter = 0;
	let pendingShare = false;
	let inTransferPair = false;
	let prevTransferLabel = null;
	let prevWasAuto = false;
	let inBindingOff = false;
	function isAutoMovePass(kp) {
		return kp && kp.isAutoMove === true;
	}
	function rowLabelFor(kpass, idx, allPasses) {
		const isMove = isAutoMovePass(kpass);
		const isTransfer = (kpass.type === 'Rr-Tr' || kpass.type === 'Rl-Tr' ||
			kpass.type === 'Tr-Rr' || kpass.type === 'Tr-Rl');
		const isTuTuOut = (kpass.type === 'Tu-Tu');
		const isLeft = kpass.direction === DIRECTION_LEFT;
		const hasFront = kpass.FRNT && kpass.FRNT.indexOf('-') !== -1;
		const hasBack = kpass.REAR && kpass.REAR.indexOf('-') !== -1;
		const isAltBed = hasFront && hasBack;

		let label;
		if (inBindingOff) {
			label = `// row: ${rowCounter - 1}`;
		} else if (pendingShare) {
			label = `// row: ${rowCounter - 1}`;
			pendingShare = false;
		} else if (isMove && prevWasAuto) {
			//Consecutive auto-moves share the same row.
			label = `// row: ${rowCounter - 1}`;
		} else if (isMove) {
			label = `// row: ${rowCounter}`;
			rowCounter += 1;
			//Lookahead: if the pattern is [auto, real, auto], the middle
			//real takes a fresh row (no share). Otherwise the next pass
			//shares with this auto-move.
			const next1 = allPasses[idx + 1];
			const next2 = allPasses[idx + 2];
			const altPattern = next1 && !isAutoMovePass(next1) && next2 && isAutoMovePass(next2);
			pendingShare = !altPattern;
			inTransferPair = false;
		} else if (isTuTuOut) {
			label = `// row: ${rowCounter - 1}`;
			inBindingOff = true;
		} else if (inTransferPair) {
			label = prevTransferLabel;
			rowCounter += 1;
			inTransferPair = false;
		} else if (isTransfer) {
			label = `// rows: ${rowCounter}-${rowCounter + 1}`;
			rowCounter += 1;
			inTransferPair = true;
			prevTransferLabel = label;
		} else if (isLeft && isAltBed) {
			label = `// rows: ${rowCounter}-${rowCounter + 1}`;
			rowCounter += 1;
		} else {
			label = `// row: ${rowCounter}`;
	...
	kcodePasses.forEach(function(kpass, kpassIdx){
		//console.log("Doing:", kpass); //DEBUG
		if ('pauseMessage' in kpass) {
			out(kpass.pauseMessage);
		}
		//PATCH (knitlab2): App emits 3 `//` separators + 1 `// row: N`
		//label per pass. Reduce from the legacy 4 separators to 3 so the
		//row label replaces the 4th.
		out("//"); out("//"); out("//");
		out(rowLabelFor(kpass, kpassIdx, kcodePasses));
		if ('comment' in kpass) {
			out("// " + kpass.comment);
		}
